...
const Attendance = () => {
	...
	const handleDelete = (record) => {
		deleteAttendance(record?.id, {
			...
			onError: () =>
				notify({ message: 'Unable to delete attendance!', status: 'error', toastOptions: { toastId: 'attendance_deletion_failure' } }),
		});
	};
	...
	const handleAttendanceCreation = (event) => {
		...
		createAttendance(data, {
			...
			onError: (error) => {
				console.log(error);
				notify({
					message: error?.response?.data?.detail || 'Unable to create attendance!',
					status: 'error',
	...
	const handleAttendanceEditing = (event) => {
		...
		updateAttendance(
			{ id: formData.id, data },
			{
				...
				onError: (error) =>
					notify({
						message: error?.response?.data?.detail || 'Unable to update attendance!',
						status: 'error',
